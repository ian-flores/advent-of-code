...
func main() {
	...
	for i := 0; i < len(stacks); i++ {
		stacks[i] = stack_reversal(stacks[i])
	}
	...
	for line := 10; line < len(fileLines); line++ {
		...
		provisional_stack := Stack{}

		for i := 0; i < boxes; i++ {
			provisional_stack.Push(stacks[giving_stack-1].Pop())
		}

		for i := 0; i < boxes; i++ {
			stacks[receiving_stack-1].Push(provisional_stack.Pop())
		}
	}
...
func stack_reversal(stack Stack) Stack {
